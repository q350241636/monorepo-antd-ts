...
const RoleButton = memo(
  ({ index, onButtonClick }: { index: number; onButtonClick: (type: string, btnIndex: number) => void }) => (
    <>
      <Button size="small" style={{ marginRight: '10px' }} onClick={() => onButtonClick('editMenu', index)} type="link">
        编辑权限
      </Button>
...
function RoleManage() {
  ...
  return (
    <PageWrap>
      {/* 查询表单 */}
      <SearchForm formList={formList} actions={actions} onSearch={onSearch} onClick={onAddRole} />
      {editVisible && (
        <AddOrEdit role={currentRole} visible={editVisible} onClose={closeEditModal} onConfirm={onOkEditModal} />
      )}
      {editMenuVisible && (
        <EditMenu
      ...
      <BaseTable<Role> data={roleData} onChange={onTableChange} loading={loading}>
        <Table.Column<Role> title="id" dataIndex="id" align="center" />
        <Table.Column<Role> title="角色名称" dataIndex="name" align="center" />
        <Table.Column<Role>
          title="操作"
          align="center"
          render={(_text, _record, index) => <RoleButton index={index} onButtonClick={onButtonClick} />}
        />
      </BaseTable>
    </PageWrap>
  )
