...
function AddOrEditUser(props: AddOrEditUserProps) {
  ...
  const onOk = useCallback(() => {
    form.validateFields().then(res => {
      ...
      if (info.id) {
        apiUpdateUser(info).then(() => {
          message.success('修改成功')
          props.onConfirm()
        })
      } else {
        apiCreateUser(info).then(() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Form.Item label="手机号码" name="mobile" rules={[{ required: true, message: '请输入用户手机号' }]}>
          <Input />
        </Form.Item>
        <Form.Item label="用户角色" name="roleId" rules={[{ required: true, message: '请选择用户角色' }]}>
          <Select placeholder="请选择用户角色">
            {roleList.map((role: Role) => (
              <Select.Option key={role.id} value={role.id ?? 0}>
                {role.name}
              </Select.Option>
            ))}
          </Select>
