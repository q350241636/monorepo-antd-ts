...
const UserButton = memo(
  ({ index, onButtonClick }: { index: number; onButtonClick: (type: string, btnIndex: number) => void }) => (
    <>
      <Button size="small" style={{ marginRight: '10px' }} onClick={() => onButtonClick('edit', index)} type="link">
        编辑
      </Button>
...
function UserManage() {
  ...
  return (
    <PageWrap>
      {/* 查询表单 */}
      <SearchForm formList={formList} actions={actions} onSearch={onSearch} onClick={onAddUser} />
      {editVisible && (
        <AddOrEdit user={currentUser} visible={editVisible} onClose={closeEditModal} onConfirm={onOkEditModal} />
      )}
      {/* 数据表格 */}
      <BaseTable<User> data={userData} onChange={onTableChange} loading={loading}>
        <Table.Column<User> title="用户id" dataIndex="id" align="center" />
        <Table.Column<User> title="用户账号" dataIndex="account" align="center" />
        <Table.Column<User> title="用户手机号" dataIndex="mobile" align="center" />
        <Table.Column<User> title="用户头像" dataIndex="avatar" align="center" render={text => <Avatar src={text} />} />

        <Table.Column<User>
          title="用户角色"
          dataIndex="role"
          align="center"
          render={(_text, record) => (record.role ? record.role.name : '无')}
        />
        <Table.Column<User>
          title="用户状态"
          dataIndex="status"
          align="center"
          render={text => <Tag color={text === 1 ? 'green' : 'red'}>{text === 1 ? '启用' : '禁用'}</Tag>}
        />
        <Table.Column<User>
          title="操作"
          align="center"
          render={(_text, _record, index) => <UserButton index={index} onButtonClick={onButtonClick} />}
        />
      </BaseTable>
    </PageWrap>
  )
