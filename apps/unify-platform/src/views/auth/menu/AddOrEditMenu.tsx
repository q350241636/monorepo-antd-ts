...
function AddOrEditMenu(props: AddOrEditMenuProps) {
  ...
  if (menu && menu.parent) {
    menu.parentIds = [menu.parent.id ?? 0]
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Form.Item name="url" label="菜单URL" rules={[{ required: true, message: '请输入菜单url' }]}>
          <Input />
        </Form.Item>
        <Form.Item label="菜单图标" name="icon" rules={[{ required: true, message: '请输入菜单图标' }]}>
          <Input />
        </Form.Item>
        <Form.Item label="菜单排序" name="sort" rules={[{ required: true, message: '请输入菜单排序' }]}>
          <Input type="number" />
        </Form.Item>
        <Form.Item label="菜单描述" name="desc" rules={[{ required: true, message: '请输入菜单描述' }]}>
          <Input />
        </Form.Item>
